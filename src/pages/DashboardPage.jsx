...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white border border-dark-400 rounded-lg px-3 py-2 shadow-xl">
...
export default function DashboardPage({ data }) {
  const { tenant, activeAgent, agents } = useTenant()
  const allLeads = data.leads
  // Filter leads by active agent for dashboard stats
  const leads = activeAgent
    ? allLeads.filter(l => !l.agent_id || l.agent_id === activeAgent.id)
    : allLeads
  const { properties, visits, faqs } = data
        ...
        <div>
          ...
          <div className="card p-6 mb-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="font-display font-bold text-lg mb-1">Votre plan actuel</h3>
                <p className="text-sm text-dark-200">Gérez votre abonnement et vos limites d'utilisation.</p>
              </div>
              <span className="px-3 py-1 rounded-full text-xs font-bold text-white" style={{ background: '#0071e3' }}>
                Growth
              </span>
            </div>

            <div className="grid sm:grid-cols-3 gap-4 mb-6">
              <div className="p-4 rounded-xl bg-dark-700 border border-dark-400/60">
                <div className="text-[10px] text-dark-200 uppercase tracking-wider mb-2">Agents</div>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-display font-bold">{agents.length}</span>
                  <span className="text-sm text-dark-200">/ 1</span>
                </div>
                <div className="w-full h-1.5 rounded-full bg-dark-500 mt-2">
                  <div className="h-full rounded-full" style={{ width: `${Math.min(agents.length * 100, 100)}%`, background: agents.length > 1 ? '#f43f5e' : '#0071e3' }} />
                </div>
              </div>
              <div className="p-4 rounded-xl bg-dark-700 border border-dark-400/60">
                <div className="text-[10px] text-dark-200 uppercase tracking-wider mb-2">Leads ce mois</div>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-display font-bold">{kpis.totalLeads}</span>
                  <span className="text-sm text-dark-200">/ 100</span>
                </div>
                <div className="w-full h-1.5 rounded-full bg-dark-500 mt-2">
                  <div className="h-full rounded-full bg-brand-500" style={{ width: `${Math.min(kpis.totalLeads, 100)}%` }} />
                </div>
              </div>
              <div className="p-4 rounded-xl bg-dark-700 border border-dark-400/60">
                <div className="text-[10px] text-dark-200 uppercase tracking-wider mb-2">Messages</div>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-display font-bold">{msgStats.total}</span>
            ...
            <div className="flex items-center gap-3">
              <span className="text-3xl font-display font-extrabold">$299</span>
              <span className="text-sm text-dark-200">/mois</span>
              <span className="ml-auto text-xs text-dark-200">Prochain renouvellement : {fmtDate(new Date(Date.now() + 30 * 86400000).toISOString())}</span>
            </div>
          </div>

          {/* Upgrade options */}
          <h3 className="font-display font-bold text-base mb-4">Passer au niveau supérieur</h3>
          <div className="grid md:grid-cols-2 gap-4">
            {[
              {
                name: 'Scale', price: '$799', period: '/mois', tagline: 'Dominez votre marché', popular: true,
                features: ['5 agents personnalisables', 'Builder complet + IA', 'Leads illimités', '10 numéros WhatsApp', 'Analytics avancé'],
              },
              {
                name: 'Enterprise', price: 'Sur mesure', period: '', tagline: 'Pour les leaders', popular: false,
                features: ['Agents illimités', 'API + Custom nodes', 'Support dédié + SLA', 'Formation équipe', 'Custom branding'],
              },
            ].map((plan, i) => (
              <div key={i} className={`card p-5 relative ${plan.popular ? 'border-brand-500/30' : ''}`}>
                {plan.popular && (
                  <div className="absolute -top-2.5 left-4 px-3 py-0.5 rounded-full text-[10px] font-bold text-white" style={{ background: '#0071e3' }}>
                    Recommandé
                  </div>
                )}
                <h4 className="font-display font-bold text-lg mt-1">{plan.name}</h4>
                <p className="text-xs text-dark-200 mb-3">{plan.tagline}</p>
                <div className="flex items-baseline gap-1 mb-4">
                  <span className="text-2xl font-display font-extrabold">{plan.price}</span>
                  {plan.period && <span className="text-sm text-dark-200">{plan.period}</span>}
                </div>
                <ul className="space-y-2 mb-5">
                  {plan.features.map((f, fi) => (
                    <li key={fi} className="flex items-center gap-2 text-xs text-dark-100">
                ...
                <button className={`w-full py-2.5 rounded-lg text-sm font-semibold transition-all ${
                  plan.popular
                    ...
                    : 'bg-dark-500 text-dark-100 border border-dark-400 hover:border-dark-300'
                }`} style={plan.popular ? { background: '#0071e3' } : {}}>
                  {plan.popular ? 'Passer à Scale →' : 'Nous contacter →'}
                </button>
              </div>
            ))}
          </div>
